...
const package = JSON.parse(fs.readFileSync("./vscode/settings.json"));
const defaultBindings = package["vspacecode.bindingOverrides"];
...
function getKeyTypeOrder(b) {
  ...
  const len = b.key ? Array.from(b.key).length : Array.from(b.keys).length;
  if (len === 1) {
    return 0;
  } else if (/.+-.+/g.test(b.key) || /.+-.+/g.test(b.keys)) {
    return 3;
  } else {
    return 2;
  }
...
function compareFunction(a, b) {
  ...
  a_key = a.key ? a.key : a.keys;
  b_key = b.key ? b.key : b.keys;
  diff = compareKeyString(a_key, b_key);
  return diff;
}
