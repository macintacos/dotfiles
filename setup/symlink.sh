...
nodenv_link() {
	log info "Linking nodenv config..."
	log info "Clearing target files.."
	rm -f "$(nodenv root)/default-packages"

	log info "Linging kitty config..."
	ln -sF "${DOTFILES_HOME}/backup/default-packages" "$(nodenv root)/default-packages"

	log ok "default-packages for nodenv linked. You should run 'nodenv default-packages install --all'"
}
